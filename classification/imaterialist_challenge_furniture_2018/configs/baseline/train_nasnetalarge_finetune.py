...
from models.nasnet_a_large import FurnitureNASNetALargeFinetunned
...
MODEL = FurnitureNASNetALargeFinetunned(pretrained='imagenet')

N_EPOCHS = 100

OPTIM = SGD(
    params=[
        {"params": MODEL.features_12_17.parameters(), 'lr': 0.005},
        {"params": MODEL.classifier.parameters(), 'lr': 0.1},
    ],
    momentum=0.9)

LR_SCHEDULERS = [
    ExponentialLR(OPTIM, gamma=0.95)
]

REDUCE_LR_ON_PLATEAU = ReduceLROnPlateau(OPTIM, mode='min', factor=0.1, patience=5, threshold=0.07, verbose=True)
